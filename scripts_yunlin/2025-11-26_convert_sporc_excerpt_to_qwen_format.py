...
This version generates customized prompts per sample with:
...
OUTPUT_FILE = Path("/home/ubuntu/image-to-text/Qwen3-VL/data/qwen_training_data_sporc_4004_samples_v2.json")

# Prompt template with placeholder for word count range
PROMPT_TEMPLATE = """You are creating a podcast transcript between two hosts based on the images I uploaded. Based on the visual information from these images, create an engaging conversational podcast transcript.

The hosts should discuss the topics, experiences, or themes that connect to what's in the images. They are talking about topics related to visuals in the photos rather than they are looking at the photos. DO NOT mention "images", "photos", "pictures". They can ask questions.

The word count should be around {word_range} words."""
...
def main():
    parser = argparse.ArgumentParser(description='Convert SPoRC excerpts to Qwen training format (v2 with dynamic prompts)')
    ...
    print("=" * 80)
    print("Converting SPoRC excerpts to Qwen3-VL format (v2 with dynamic prompts)")
    print("=" * 80)
    print(f"Output file: {output_file}")
    print(f"Data sources: {len(data_sources)}")
    print(f"Prompt template: {PROMPT_TEMPLATE[:60]}...")
