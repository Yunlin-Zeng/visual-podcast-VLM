...
# =============================================================================
# Fine-tune Qwen3-VL-32B on SPoRC - v3 with 894-sample style prompt
#
# Key changes from v2 (conservative):
#   - Prompt style: 894-sample style ("Speaker 1:, Speaker 2:" format)
#   - "without introductions or sign-offs" instruction
#   - Dynamic word count range per sample
#
# Dataset: 4004 samples (sporc_4004_samples_v3)
# Hardware: 8 GPUs
# =============================================================================
...
# Dataset configuration - v3 with 894-sample style prompt
DATASET="sporc_4004_samples_v3%100"

# Output configuration with date prefix
DATE=$(date +%Y-%m-%d)
RUN_NAME="qwen3vl-32b-sporc-v3"
OUTPUT_DIR="/home/ubuntu/image-to-text/Qwen3-VL/finetuned_models/${DATE}_${RUN_NAME}"
LOG_FILE="${OUTPUT_DIR}/training.log"

# =============================================================================
# HYPERPARAMETERS - SAME AS SUCCESSFUL 894-SAMPLE RUN
# =============================================================================
LR=4e-6                   # SAME as successful run
...
WEIGHT_DECAY=0.1          # SAME as successful run
WARMUP_RATIO=0.1          # SAME as successful run
NEFTUNE_NOISE_ALPHA=5.0   # SAME as successful run (prevents memorization)

# Save steps calculation:
# 4004 samples / 32 effective batch = 125.125 steps per epoch
# Save at ~half epoch (62) and end (~126)
SAVE_STEPS=62

# LoRA configuration - SAME as successful run
USE_LORA=True
LORA_R=16
LORA_ALPHA=32
LORA_DROPOUT=0.05
...
# Print configuration
echo "================================================================================"
echo "Qwen3-VL-32B Fine-tuning v3 (894-sample style prompt)"
echo "================================================================================"
echo "Model: ${MODEL_PATH}"
echo "Dataset: ${DATASET} (4004 samples)"
echo ""
echo "Prompt style: 894-sample format"
echo "  - Speaker 1:, Speaker 2:, Speaker 3: format"
echo "  - 'without introductions or sign-offs'"
echo "  - Dynamic word count range per sample"
echo ""
echo "Hyperparameters (SAME as successful 894-sample run):"
...
echo "  Steps per epoch: ~$((4004 / (BATCH_SIZE * GRAD_ACCUM_STEPS * NPROC_PER_NODE)))"
